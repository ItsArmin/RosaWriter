...
class TemplateRenderer {
  ...
  func findTemplate(mood: StoryMood, theme: StoryTheme, preferredPageCount: Int? = nil)
    -> StoryTemplate?
  {
    ...
    if let mapping = Self.templateMappings[key] {
      ...
      if !candidates.isEmpty {
        print(
          "📖 [TemplateRenderer] Using mapped template: \(key) → \(mapping.mood.rawValue)+\(mapping.theme.rawValue)"
        )
  ...
  private func selectByPageCount(from candidates: [StoryTemplate], preferredPageCount: Int?)
    -> StoryTemplate?
  {
